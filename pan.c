...
#define Index(x, y)	Boundcheck(x, y, 0, 0, 0)
	...
	case 1:	/* :init: */
		...
		/* params: */
		/* locals: */
		((P1 *)pptr(h))->c01 = addqueue(1, 0);
		((P1 *)pptr(h))->c12 = addqueue(2, 0);
		((P1 *)pptr(h))->c20 = addqueue(3, 0);
#ifdef VAR_RANGES
#endif
#ifdef HAS_CODE
		locinit1(h);
	...
	case 0:	/* process1 */
		...
		((P0 *)pptr(h))->R = par2;
		/* locals: */
		((P0 *)pptr(h))->d = ((P0 *)this)->id;
		((P0 *)pptr(h))->e = 0;
		((P0 *)pptr(h))->f = 0;
		((P0 *)pptr(h))->active_iters = 0;
#ifdef VAR_RANGES
		logval("process1:id", ((P0 *)pptr(h))->id);
		logval("process1:d", ((P0 *)pptr(h))->d);
		logval("process1:e", ((P0 *)pptr(h))->e);
		logval("process1:f", ((P0 *)pptr(h))->f);
		logval("process1:active_iters", ((P0 *)pptr(h))->active_iters);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(void)
{
		...
		now.max_messages = 9;
#ifdef VAR_RANGES
		logval("iters_exceeded", now.iters_exceeded);
		logval("leader", now.leader);
		logval("numleaders", now.numleaders);
		logval("highest_id", now.highest_id);
		logval("num_messages", now.num_messages);
		logval("max_messages", now.max_messages);
#endif
...
{	int j=0, i = now._nr_qs;
	...
	switch (n) {
	case 3: j = sizeof(Q3); q_flds[3] = 1; q_max[3] = 16; break;
	case 2: j = sizeof(Q2); q_flds[2] = 1; q_max[2] = 16; break;
	case 1: j = sizeof(Q1); q_flds[1] = 1; q_max[1] = 16; break;
...
{	if (!from--)
	...
	switch(((Q0 *)qptr(from))->_t) {
	case 3: return (q_sz(from) == 16);
	case 2: return (q_sz(from) == 16);
	case 1: return (q_sz(from) == 16);
	case 0: printf("queue %d was deleted\n", from+1);
	}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full(((P0 *)this)->L)))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full(((P0 *)this)->L)))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len(((P0 *)this)->L)))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len(((P0 *)this)->L)))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full(((P0 *)this)->R)))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full(((P0 *)this)->R)))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len(((P0 *)this)->R)))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len(((P0 *)this)->R)))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full(((P1 *)this)->c01)))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full(((P1 *)this)->c01)))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len(((P1 *)this)->c01)))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len(((P1 *)this)->c01)))
			    ) return 0; break;
		case 4: if (	(t->ty[i] == Q_FULL_F && ( q_full(((P1 *)this)->c12)))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full(((P1 *)this)->c12)))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len(((P1 *)this)->c12)))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len(((P1 *)this)->c12)))
			    ) return 0; break;
		case 5: if (	(t->ty[i] == Q_FULL_F && ( q_full(((P1 *)this)->c20)))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full(((P1 *)this)->c20)))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len(((P1 *)this)->c20)))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len(((P1 *)this)->c20)))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	int    leader:	%d\n", now.leader);
	printf("	int    numleaders:	%d\n", now.numleaders);
	printf("	int    highest_id:	%d\n", now.highest_id);
	printf("	int    num_messages:	%d\n", now.num_messages);
	printf("	int    max_messages:	%d\n", now.max_messages);
	printf("	bit    iters_exceeded:	%d\n", now.iters_exceeded);
}
void
...
{	/* int i; */
	...
	printf("	chan c01 (=%d):	len %d:\t", ((P1 *)pptr(pid))->c01, q_len(((P1 *)pptr(pid))->c01));
	c_chandump(((P1 *)pptr(pid))->c01);
	printf("	chan c12 (=%d):	len %d:\t", ((P1 *)pptr(pid))->c12, q_len(((P1 *)pptr(pid))->c12));
	c_chandump(((P1 *)pptr(pid))->c12);
	printf("	chan c20 (=%d):	len %d:\t", ((P1 *)pptr(pid))->c20, q_len(((P1 *)pptr(pid))->c20));
	c_chandump(((P1 *)pptr(pid))->c20);
		break;
	case 0:
		printf("local vars proc %d (process1):\n", pid);
	printf("	int    id:	%d\n", ((P0 *)pptr(pid))->id);
	printf("	chan L (=%d):	len %d:\t", ((P0 *)pptr(pid))->L, q_len(((P0 *)pptr(pid))->L));
	c_chandump(((P0 *)pptr(pid))->L);
	printf("	chan R (=%d):	len %d:\t", ((P0 *)pptr(pid))->R, q_len(((P0 *)pptr(pid))->R));
	c_chandump(((P0 *)pptr(pid))->R);
	printf("	int    d:	%d\n", ((P0 *)pptr(pid))->d);
	printf("	int    e:	%d\n", ((P0 *)pptr(pid))->e);
	printf("	int    f:	%d\n", ((P0 *)pptr(pid))->f);
	printf("	int    active_iters:	%d\n", ((P0 *)pptr(pid))->active_iters);
		break;
	}
}
void
